...
function getOpenAIClientConfig(apiKey: string) {
...
export async function processMessage(
  ...
  if (componentIntent.triggerPDF && detectStructuredPdfIntent(request.message)) {
    ...
    const pdfModel = "meta-llama/llama-3.3-70b-instruct";
  ...
  if (componentIntent.triggerQuiz && detectStructuredQuizIntent(request.message)) {
    ...
    const quizModel = "meta-llama/llama-3.3-70b-instruct";
  ...
  if (model === "apex-omni") {
    ...
    const omniActualModel = "meta-llama/llama-3.3-70b-instruct";
...
async function callCerebras(
  ...
  for (const { key, name } of keysToTry) {
    try {
      console.log(`[ai-orchestrator] Trying API provider: ${name}`);
      const config = getOpenAIClientConfig(key);
      const client = wrapOpenAIClient(new OpenAI(config));
...
Do not repeat or generate another markdown image for this URL in your response content. The system handles image presentation.`;
      ...
      console.log(`🚀 Attempting API call with model ${actualModel} via ${name}... (stream: ${!!onChunk})`);
      ...
      if (quizIntent && onChunk) {
        ...
        console.log(`✅ ${name} quiz response normalized and streamed.`);
      ...
      if (onChunk) {
        ...
        console.log(`✅ ${name} stream succeeded!`);
      ...
      console.warn(`⚠️ API provider ${name} failed:`, err.message || err);
      lastError = err;
      // Continue to next provider in loop
    }
  }

  const errorMsg = lastError?.message || lastError || "Unknown API provider error";
  throw new Error(`All configured API providers failed. Last error: ${errorMsg}`);
}
...
export async function generateMcqResponse(
  ...
  const actualModel = "meta-llama/llama-3.3-70b-instruct";
